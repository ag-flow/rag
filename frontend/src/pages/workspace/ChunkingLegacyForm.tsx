...
import { useUpsertChunkingConfig } from "@/hooks/useChunking";
...
/**
 * Formulaire du moteur `legacy` (pipeline historique en caractères) :
 * stratégie textuelle, max/min/overlap_chars et options de nettoyage.
 */
export function ChunkingLegacyForm({ workspaceName, config }: Props) {
  const { t } = useTranslation("workspace");
  const { toast } = useToast();
  const upsert = useUpsertChunkingConfig(workspaceName);
  ...
  const onSubmit = (values: ChunkingFormValues) => {
    ...
    const payload: ChunkingSpec = {
      ...
      extras: computeExtrasPayload(values.strategy, cleaning, config),
    };
    upsert.mutate(
      ...
      >
        ...
        <p className="text-xs text-slate-500">
          {t("chunking.lastModified", {
            when: formatRelativeTime(config.updated_at, t),
          })}
        </p>
        ...
        <div className="flex items-center justify-end gap-2 pt-2">
          <Button
            type="button"
            variant="ghost"
            onClick={() => form.reset(configToForm(config))}
