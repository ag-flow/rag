...
async def _seed_push_job(pool: asyncpg.Pool, ws_name: str) -> tuple[str, object]:
    async with pool.acquire() as conn:
        ...
        job_id = await conn.fetchval(
            ...
            "VALUES ($1, 'push', 'pending', 'corr-cb-001') RETURNING id",
            ws_id,
        )
        await conn.execute(
...
@pytest.mark.asyncio
async def test_blocking_error_opens_circuit_breaker(
    pool: asyncpg.Pool, tmp_path: Path
) -> None:
    job_id, ws_id = await _seed_push_job(pool, "ws_cb1")
...
@pytest.mark.asyncio
async def test_picker_skips_workspace_with_open_circuit(
    pool: asyncpg.Pool, tmp_path: Path
) -> None:
    """Un job pending dont le workspace a un circuit ouvert ne doit pas être piké."""
    async with pool.acquire() as conn:
        ws_id = await seed_workspace(conn, name="ws_cb2")
        await conn.execute(
            "INSERT INTO indexer_configs (workspace_id, provider, model, dimension) "
            "VALUES ($1, 'openai', 'text-embedding-3-small', 1536)",
            ws_id,
        ...
        job_id = await conn.fetchval(
            "INSERT INTO index_jobs (workspace_id, triggered_by, status, correlation_id) "
            "VALUES ($1, 'push', 'pending', 'corr-cb-002') RETURNING id",
            ws_id,
        )
        await conn.execute(
            "INSERT INTO push_job_payloads (job_id, path, content) VALUES ($1, $2, $3)",
        ...
        # Circuit ouvert manuellement (open_until = futur lointain)
        await conn.execute(
            """
            INSERT INTO indexer_circuit_breakers
                (workspace_id, provider, model, error_message, open_until)
            VALUES ($1, 'openai', 'text-embedding-3-small', 'no credits',
                    now() + interval '1 hour')
            """,
            ws_id,
        )
    ...
    result = await execute_next_pending_job(
        config_pool=pool,
        storage=RepoStorage(tmp_path),
        indexer=_FailingIndexer(RuntimeError("should not be called")),  # type: ignore[arg-type]
        resolver=_StubResolver(),  # type: ignore[arg-type]
        client_provider=_StubClientProvider(),  # type: ignore[arg-type]
        webhook_secret=None,
    )
    # Aucun job piké (circuit ouvert)
...
@pytest.mark.asyncio
async def test_auth_error_opens_circuit_breaker(
    pool: asyncpg.Pool, tmp_path: Path
) -> None:
    _, ws_id = await _seed_push_job(pool, "ws_cb3")
