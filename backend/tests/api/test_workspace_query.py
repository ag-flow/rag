...
def test_read_endpoints_reject_missing_bearer(
    ...
    r = _get(admin_client, "/index-status", key=None, params={"workspace": "wq_noauth"})
...
def test_read_endpoints_reject_invalid_key(
    ...
    r = _get(
        admin_client, "/index-status", key="not-a-real-key", params={"workspace": "wq_badkey"}
    )
    assert r.status_code == 401
...
def test_read_scope_key_is_accepted(
    ...
    r = _get(admin_client, "/index-status", key=key, params={"workspace": "wq_read"})
...
def test_unknown_workspace_returns_401(
    ...
    r = _get(admin_client, "/index-status", key=key, params={"workspace": "ghost"})
    assert r.status_code == 401
...
def test_files_search_empty_returns_zero(
    ...
    r = _get(
        admin_client, "/files", key=key, params={"workspace": "wq_files", "pattern": "anything"}
    )
    assert r.status_code == 200, r.text
    body = r.json()
    assert body["count"] == 0
...
def test_document_not_indexed_returns_404(
    ...
    r = _get(
        admin_client, "/documents", key=key, params={"workspace": "wq_doc", "path": "nope.md"}
    )
    assert r.status_code == 404
    assert r.json()["detail"] == "document_not_indexed"
...
def test_jobs_list_empty_returns_200(
    ...
    r = _get(admin_client, "/jobs", key=key, params={"workspace": "wq_jobs"})
    assert r.status_code == 200, r.text
    assert r.json() == []
...
def test_chunking_config_read_returns_200(
    ...
    r = _get(admin_client, "/chunking-config", key=key, params={"workspace": "wq_chunk"})
    assert r.status_code == 200, r.text
    body = r.json()
...
def test_rerank_not_configured_returns_404(
    ...
    r = _get(admin_client, "/rerank", key=key, params={"workspace": "wq_rerank"})
    assert r.status_code == 404
...
def test_hybrid_not_configured_returns_404(
    ...
    r = _get(admin_client, "/hybrid-config", key=key, params={"workspace": "wq_hybrid"})
    assert r.status_code == 404
